...
class Backtest:
    ...
    def load_params_from_cfg(self):
        ...
        indicators.read([self.get_strategy_path() + "parametres.cfg", self.get_strategy_path() + "config-bot.cfg"])
    ...
    def load_configs_from_cfg(self):
        ...
        configs.read([self.get_strategy_path() + "parametres.cfg", self.get_strategy_path() + "config-bot.cfg"])
    ...
    def load_ohlc(
        ...
        try :
            ...
            print(f"Erreur lors de l'ouverture du fichier {self.get_strategy_path() + 'pair_list.json'} : {err}")
            exit(5)
        if end_date == None:
        ...
        os.makedirs(self.get_strategy_path()+f"../dependencies/database/FTX/{timeframe}", exist_ok=True)
        if self.showLog==True :
            print(
        ...
        if useWhitelist == False:
            ...
            cryptoNameList = getCryptoListFromDf(df, int(self.get_configs()['STRATEGIE']['classementminimum']))
        else:
        ...
        if useBlacklist==True :
            for pair in pairJson["blacklist"] :
                if pair in cryptoNameList :
                    ...
                    if self.showLog==True :
                        print(pair, "ne sera pas utilisée car appartient à la blacklist.")
        if self.showLog==True :
            print(
    ...
    def internal_load_indicators(self, forceRedownload=False, indicators=None):
        ...
        if self.dfList == None or forceRedownload==True :
            ...
            self.dfList = load_indicators(self.ohlc, self.ohlc, self.get_params()['params'], self.showLog)
            if self.showLog==True :
    ...
    def run(self, showLog=True, params=None, startingBalance=None) :
                        ...
                        try:
                            ...
                            if prixDeVente != 0 :
                                ...
                                if raison=="Takeprofit" :
                                    frais = quantiteDeCoinDuneCrypto[i]*prixDeVente*takerFrais
                                else :
                                    frais = quantiteDeCoinDuneCrypto[i]*prixDeVente*takerFrais
                                quantiteUSD = quantiteUSD + (gain_perte+prixAchat[i]*quantiteDeCoinDuneCrypto[i])-frais
                                #-- LOG --
                                if showLog:
                                    print(index, "| CLOSE LONG  (", round(gain_perte-frais,5), "$) :",
                                        round(quantiteDeCoinDuneCrypto[i],5), cryptoNameList[i].split('-')[0], "at",
                                        round(prixDeVente,5), '$ :', raison)
                    ...
                    if quantiteDeCoinDuneCryptoShorted[i] != 0 and useShort==True :
                        try:
                            ...
                            if prixDeVente != 0 :
                                ...
                                if raison=="Takeprofit" :
                                    frais = prixDeVente*quantiteDeCoinDuneCryptoShorted[i]*makerFrais
                                else :
                                    frais = prixDeVente*quantiteDeCoinDuneCryptoShorted[i]*takerFrais
                                quantiteUSD = quantiteUSD + (gain_perte+prixAchat[i]*quantiteDeCoinDuneCryptoShorted[i])-frais
                                #-- LOG --
                                if showLog:
                                    print(index, "| CLOSE SHORT (", round(gain_perte,5), "$) :",
                                        round(quantiteDeCoinDuneCryptoShorted[i],5), cryptoNameList[i].split('-')[0], "at",
                                        round(prixDeVente,5), '$ :', raison)
                                #-- On rénitialise les variables après la vente --
                    ...
                    try :
                        ...
                        if useLong==True and openLongCondition(row, previousRow, self.dfList, self.get_params()['params']) and activePositions < maxPositions and quantiteDeCoinDuneCryptoShorted[i]==0 and quantiteDeCoinDuneCrypto[i]==0:
                            try :
                                ...
                                usdMultiplier = float(self.configs['STRATEGIE']['usdAllocated'])/(maxPositions-activePositions)
                                ...
                                equivalentUSDdeCryptoEnPossession[i] = coin * row['close']
                                activePositions += 1
                                openLongPositions.append(cryptoNameList[i])

                                if self.useStoploss:
                                    stopLoss[i] = getStoploss("long", prixAchat[i], row, previousRow, self.dfList, self.get_params()['params'])
                                # -- Create a Take Profit --
                                if self.useTakeProfit:
                                    takeProfit[i] = getTakeprofit("long", prixAchat[i], row, previousRow, self.dfList, self.get_params()['params'])
                                # -- LOG --
                                if showLog:
                                ...
                                print(traceback.format_exc())
                                break
                        if useShort==True and openShortCondition(row, previousRow, self.dfList, self.get_params()['params']) and activePositions < maxPositions and quantiteDeCoinDuneCryptoShorted[i]==0 and quantiteDeCoinDuneCrypto[i]==0:
                            try :
                                prixAchat[i] = row['close']
                                # -- Define size of the position --
                                usdMultiplier = float(self.configs['STRATEGIE']['usdAllocated'])/(maxPositions-activePositions)
                                ...
                                # -- Create a Stop Loss --
                                if self.useStoploss:
                                    stopLoss[i] = getStoploss("short", prixAchat[i], row, previousRow, self.dfList, self.get_params()['params'])
                                # -- Create a Take Profit --
                                if self.useTakeProfit:
                                    takeProfit[i] = getTakeprofit("short", prixAchat[i], row, previousRow, self.dfList, self.get_params()['params'])
                                # -- LOG --
                                if showLog:
        ...
        if showLog:
            ...
            print(len(positionsSkipped), "opportunités d'ouvertures de positions n'ont pu être prise car maxOpenPositions était atteint")
            if useLong==True :
