...
class Backtesting():
    ...
    def simple_spot_backtest_analys(self, dfTrades, dfTest, pairSymbol, timeframe):
        ...
        dfTrades['resultat'] = dfTrades['prixAchat']-dfTrades['prixVente']
        dfTrades['resultat%'] = ((dfTrades['prixVente'] - dfTrades['prixAchat']) / dfTrades['prixAchat']) * 100
        dfTrades.loc[dfTrades['position'].str.contains("OPEN"), 'resultat'] = None
        dfTrades.loc[dfTrades['position'].str.contains("OPEN"), 'resultat%'] = None
    ...
    def multi_spot_backtest_analys(self, dfTrades, dfTest, pairList, timeframe):
        ...
        ## MODIF pour avoir les bons chiffres dans le print

        totalTrades_vrai = totalTrades / 2
        totalBadTrades_vrai = totalBadTrades // 2
        totalGoodTrades_vrai = totalGoodTrades // 2
        winRateRatio_vrai = (totalGoodTrades_vrai/totalTrades_vrai) * 100
        ...
        print("\n----- Trades Informations -----")
        print("Total trades on period ALLO :", totalTrades_vrai)
        print("Number of positive trades :", totalGoodTrades_vrai)
        print("Number of negative trades : ", totalBadTrades_vrai)
        print("Trades win rate ratio :", round(winRateRatio_vrai, 2), '%')
        print("Average trades performance :", tradesPerformance, "%")
        print("Average positive trades :", AveragePercentagePositivTrades, "%")
        print("Average negative trades :", AveragePercentageNegativTrades, "%")
    ...
    def multi_spot_backtest_print(self, dfTrades, dfTest, pairList, timeframe):
       # -- BackTest Analyses --
        dfTrades = dfTrades.set_index(dfTrades['date'])
        dfTrades.index = pd.to_datetime(dfTrades.index)
        dfTrades['resultat'] = dfTrades['prixAchat']-dfTrades['prixVente']
        ...
        print("[" + str(dfTest.index[0]) + "] -> [" +
                str(dfTest.index[len(dfTest)-1]) + "]") #interval date

        print(round(wallet, 2)) #final balance
        print(round(algoPercentage, 2)) #Performance vs US Dollar (%) :
        print(round(holdPercentage, 2)) #Bitcoin Buy and Hold Performence (%)
        print(round(vsHoldPercentage, 2)) # Performance vs Buy and Hold : %
        print(bestTrade) #best trade %
        print(worstTrade) #pire trade %
        print(round(100*dfTrades['drawDownPct'].max(), 2)) # pire draw down
        print(round(dfTrades['frais'].sum(), 2)) #taxes totales $

        print(totalTrades)
        print(totalGoodTrades) #trades positifs
        print(totalBadTrades) #trades negatifs
        print(round(winRateRatio, 2)) #trades winrate ratio
        print(tradesPerformance) #Average trades performance %
        print(AveragePercentagePositivTrades) #Average positive trades %
        print(AveragePercentageNegativTrades) #Average negative trades %
        ...
        for pair in pairList:
            try:
                ...
                pairMax = str(round(dfPairLoc.max(),2))+' %'
                pairWinRate = str(round(100*(pairGoodTrade/pairTotalTrade),2))+' %'
                #print('{:<6d}{:>10s}{:>15s}{:>15s}{:>15s}{:>15s}{:>15s}'.format(pairTotalTrade,pair,pairResult,pairAverage,pairMin,pairMax,pairWinRate))
            except:
                pass
    ...
    def giveMeWinRatio(self, dfTrades, dfTest, pairList, timeframe):
        # -- BackTest Analyses --
        dfTrades = dfTrades.set_index(dfTrades['date'])
        dfTrades.index = pd.to_datetime(dfTrades.index)
        dfTrades['resultat'] = dfTrades['prixAchat']-dfTrades['prixVente']
        ...
        for pair in pairList:
            try:
                ...
                pass
        print(tradesPerformance)
        return tradesPerformance
        
    def giveMeFinalBalance(self, dfTrades, dfTest, pairList, timeframe):
        # -- BackTest Analyses --
        dfTrades = dfTrades.set_index(dfTrades['date'])
        dfTrades.index = pd.to_datetime(dfTrades.index)
        dfTrades['resultat'] = dfTrades['prixAchat']-dfTrades['prixVente']
    ...
    def past_simulation(
        ...
        dfTrades['resultat'] = dfTrades['prixAchat']-dfTrades['prixVente']
        dfTrades['resultat%'] = ((dfTrades['prixVente'] - dfTrades['prixAchat']) / dfTrades['prixAchat']) * 100
        dfTrades = dfTrades.loc[dfTrades['position'].str.contains("CLOSE"),'resultat%']
        dfTrades = dfTrades + 1
