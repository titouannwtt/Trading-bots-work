...
class DataEngine():
    ...
    def get_historical_from_db_end(self, symbol, timeframe, start_date, end_date):
        '''
        Get the OHLCV from local pickle file
        Works for all exchange in database
        ...
        symbol = symbol.replace('/','')
        try:
            path=self.path_to_data+'/'+self.exchange_name+'/'+timeframe+'/'+symbol+'.p', 'rb'
            path = path.replace('databaseFTX','database/FTX')
            path = path.replace('databaseBINANCE','database/BINANCE')
            path = path.replace('databaseBinance','database/Binance')
            dbfile = open(path)	
            df = pickle.load(dbfile)
            dbfile.close()
